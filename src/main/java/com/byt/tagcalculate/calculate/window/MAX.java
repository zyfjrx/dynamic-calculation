 ...
 */
public class MAX implements Transform {
    private String size;
    private String slide;

    public MAX(List<String> params) {
        this.size = params.get(0);
        this.slide = params.get(1);
    }

    public DataStream<TagKafkaInfo> max(DataStream<TagKafkaInfo> in, String size, String slide) {
        return in
                ...
                .name("MAX-window("+size+","+slide+")");
    }

    @Override
    public void doTransForm(TStream tStream, TransformChain chain) {
        tStream.stream = max(tStream.stream,size,slide);
        chain.doTransform(tStream);
    }

    private class MaxAgg implements AggregateFunction<TagKafkaInfo, BigDecimal, BigDecimal> {
        @Override
        ...
        public BigDecimal add(TagKafkaInfo value, BigDecimal accumulator) {
            accumulator = value.getValue().compareTo(accumulator) > 0 ? value.getValue() : accumulator;
            return accumulator;
        }
    ...
    class MaxResult extends ProcessWindowFunction<BigDecimal, TagKafkaInfo, Tuple7<String, String, String, String, String, Integer, String>, TimeWindow> {
