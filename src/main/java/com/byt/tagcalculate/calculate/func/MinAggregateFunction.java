 ...
 **/
public class MinAggregateFunction implements AggregateFunction<TagKafkaInfo, Tuple2<TagKafkaInfo, BigDecimal>, TagKafkaInfo> {

    @Override
    public Tuple2<TagKafkaInfo, BigDecimal> createAccumulator() {
        return Tuple2.of(new TagKafkaInfo(), BigDecimal.valueOf(Long.MAX_VALUE));
    }

    @Override
    public Tuple2<TagKafkaInfo, BigDecimal> add(TagKafkaInfo tagKafkaInfo, Tuple2<TagKafkaInfo, BigDecimal> acc) {
        acc.f0 = tagKafkaInfo;
        acc.f1 = tagKafkaInfo.getValue().compareTo(acc.f1) == -1 ? tagKafkaInfo.getValue() : acc.f1;
        return acc;
    }
    ...
    public Tuple2<TagKafkaInfo, BigDecimal> merge(Tuple2<TagKafkaInfo, BigDecimal> acc1, Tuple2<TagKafkaInfo, BigDecimal> acc2) {
        return Tuple2.of(
                acc1.f0,
                acc1.f1.compareTo(acc2.f1) == -1 ? acc1.f1 : acc2.f1
        );
    }
}
